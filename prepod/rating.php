...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || !isset($_SESSION['login_time']) || time() - $_SESSION['login_time'] >= 7200) {
    ...
    $redirect_url = 'prepod/rating.php'; // URL текущей страницы, куда пользователь хотел перейти
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#confirmBtn").click(function() {
                ...
                $("#attandance-table tbody tr").slice(1).each(function(index) {
                    ...
                    var rat_value = $(this).find("select[name='rat_value']").val();
                ...
                $.ajax({
                    url: 'add_rating.php',
                    method: 'POST',
                    data: { ratingData: JSON.stringify(ratingData), groupId: groupId, disciplineId: disciplineId, termId: termId, classId: classId, date: date, typeId: typeId },
            ...
            $("#loadBtn").click(function() {
                ...
                if (groupId !== "" && disciplineId !== "" && classId !== "" && date !=="" && termId !== "" && typeId !=="" ) {
                    $.ajax({
                        url: 'get_students_r.php',
                        method: 'POST',
                        data: { groupId: groupId, disciplineId: disciplineId, classId: classId, date: date, termId: termId, typeId: typeId},
                        success: function(response) {
                            var datajson = JSON.parse(response);
                            if (datajson.errors) {
                                // Если сообщение об ошибке содержится в response, то скрываем кнопку и показываем только сообщение
                                Swal.fire({
                                    title: 'Ошибка',
                                    text: datajson.errors,
                                    confirmButtonText: 'ОК',
                                ...
                                // В противном случае показываем как обычно
        ...
        function validateData() {
            ...
            if (groupId && disciplineId && classId && date && typeId) {
                ...
                var output = '<p><b>Группа</b> ' + groupName + '. <b>Дисциплина:</b> ' + disciplineName + '. <b>Дата:</b> ' + date + '. <b>' + classNumber + ' пара:</b> ' + classTime + '. <b>' +  typeName + '</b></p>';
                // Отображаем строку
...
<button id="loadBtn" type="button">Показать рейтинг студентов</button>
</form>
