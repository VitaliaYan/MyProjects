...
if (!isset($_SESSION['loggedin']) || $_SESSION['loggedin'] !== true || !isset($_SESSION['login_time']) || time() - $_SESSION['login_time'] >= 7200) {
    ...
    $redirect_url = 'dekanat/send_authorize_prepod.php'; // URL текущей страницы, куда пользователь хотел перейти
...
<head>
    ...
    <title>Рассылка для авторизации преподавателей</title>
    ...
    $(document).ready(function() {
        ...
        $('.sendBtn').click(function() {
            ...
            $.ajax({
                ...
                data: { fio: fio, key: 'преподаватель'}, // Данные для отправки
...
<body>
    ...
    <div id="attandance-table-container">
        <?php
            ...
            $sql = "SELECT teachers.FIO, teachers.`E-mail` FROM attandance.teachers;";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
            $output = '<table id="attandance-table" style="background-color: #3f5f78cf; background-size: cover;">';
            $output .= '<tr><th>ФИО</th><th>Логин</th><th>Пароль</th></tr>';
                    ...
                    if ($csvFile !== false) {
                        ...
                        while (($rowCSV = fgetcsv($csvFile)) !== FALSE) {
                            if(count($rowCSV) >= 4) { // Проверяем наличие достаточного количества значений в строке
                                ...
                                if ($csvfio == $row["FIO"] && $key == 'преподаватель') {
